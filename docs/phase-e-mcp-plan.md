...
把已验证的 harvest CLI 核心能力（probe / ingest / bundle 读取）包装成 MCP 工具，
...
| 工具 | 模式 | 入参 | 出参 | 复用 |
|---|---|---|---|---|
| probe_video | 同步 | url | ProbeResult JSON | harvest.probe |
...
- extract_transcript：resolve URL → 生成 job_id → `Popen` 启动
  `python -m harvest.cli ingest <url> --no-vision --no-frame-images [flags]`（mode→flags）→
  返 job_id（不阻塞）。
- status 推断（get_*）：
  - pid 还活着 → running
...
- 子进程继承父 env（HARVEST_COOKIES_BROWSER 等由 MCP server 进程带入）。

## 4. 文件

- `harvest/mcp/__init__.py`
- `harvest/mcp/server.py`：FastMCP + 5 工具 + job store（纯函数逻辑，可单测）
- `harvest/cli.py`：加 `harvest mcp` 子命令启动 server（stdio）
- `pyproject.toml`：加 `mcp` optional dependency group
- 测试：job store 状态推断 + 工具逻辑（mock subprocess，不真跑 ingest）

## 5. 验收

- `harvest mcp` 启动 stdio server。
