...
class OCRSettings:
    ...
    touches harvest's main Python — same isolation model as the whisper-cli transcribe shim.
...
class Settings:
    ...
    # diagnostic instead of crashing ingest). Env-overridable via HARVEST_OCR_WORKER / HARVEST_OCR_VENV_PYTHON.
...
def _resolve_ocr_paths(s: "Settings") -> "Settings":
    ...
    if not s.ocr_worker_path:
        ...
        s.ocr_worker_path = str(cand) if cand.exists() else os.environ.get("HARVEST_OCR_WORKER") or None
    if not s.ocr_venv_python:
        exe = ".exe" if os.name == "nt" else ""
        cand = PROJECT_ROOT / ".ocr-venv" / "bin" / f"python{exe}"
        s.ocr_venv_python = str(cand) if cand.exists() else os.environ.get("HARVEST_OCR_VENV_PYTHON") or None
    # OCR dials are env-overridable for tuning without code edits.
    s.ocr.fps = float(os.environ.get("HARVEST_OCR_FPS", s.ocr.fps))
    s.ocr.min_conf = float(os.environ.get("HARVEST_OCR_MIN_CONF", s.ocr.min_conf))
    s.ocr.sim_threshold = float(os.environ.get("HARVEST_OCR_SIM_THRESHOLD", s.ocr.sim_threshold))
    s.ocr.diff_threshold = float(os.environ.get("HARVEST_OCR_DIFF_THRESHOLD", s.ocr.diff_threshold))
    s.ocr.detect_interval = float(os.environ.get("HARVEST_OCR_DETECT_INTERVAL", s.ocr.detect_interval))
    return s
