"""MCP interface layer (Phase E, SPEC §6): exposes harvest's verified CLI capabilities as MCP
...
Async model (minimal viable): extract_transcript spawns `harvest ingest` as a non-blocking
...
def start_ingest_job(
    ...
    """Resolve the URL, spawn a non-blocking `harvest ingest` subprocess, return its job record.

    The subprocess inherits this process's env (HARVEST_COOKIES_BROWSER / WHISPER_* must already
    ...
    cmd = [
        sys.executable, "-m", "harvest.cli", "ingest", url,
...
def main() -> int:
    """Entry point for `harvest mcp`: run the MCP server over stdio."""
    build_server().run()
    return 0
